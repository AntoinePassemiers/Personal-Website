...
title: Neural Networks: An Intuitive and Formal Introduction
...
In the age of generative AI, it may be important to understand how modern large language models, and more brodaly, neural networks, are actually trained. Neural networks have become incredibly complex over time, but much of this complexity comes from the data collection process, architectural design and implementation. In fact, the mathematics needed to understand the training of neural networks are quite simple: the core idea behind neural-network training relies mostly on derivatives and the chain rule (concepts you probably remember from high school). The notation can look intimidating, especially because we use matrices, but the underlying mechanism is simpler than it first appears.

## Neural networks are not "networks of neurons"

There is huge amount of content about neural networks and backpropagation online, so it would be pretty useless to repeat what has been written already. Anyway, when I was studying machine learning during my master's degree, I found the overwhelming majority (if not all) of formal courses and online blog posts incomplete or confusing on this topic. It took implementing a deep learning library from scratch for me to understand how things actually work. So in this post, I will explain neural networks the way I understood them.

> "What I cannot create, I do not understand."
>
> — Richard Feynman

I'm going to dispel several common misconceptions right off the bat. Do not think of neural networks as "networks of neurons". Forget about the drawings everyone is sharing, depicting neurons connected with edges all over the place. While the analogy with brains can grant you a superficial understanding of how neural networks **make predictions**, it is a dead end if your goal is to fully grasp how they are **trained**. This neural analogy is all the more false nowadays, now that neural networks involve more than fully-connected layers, such as convolutions or transformers, which do not even remotely resemble biological neurons.
...
If you don't have the time, here is a very quick crash course about matrices:
...
- When two matrices $A$ and $B$ are written down next to each other, such as $AB$, then they are multiplied in the "regular way": $\left( AB \right)_{ij} = \sum_k A_{ik} B_{kj}.$

Let's come back to the model depicted in the figure above. $XU$ is the product of matrices $X$ and $U$, which is a matrix whose elements are defined as
$$
\left( XU \right)_{ij} = \sum_k X_{ik} U_{kj}.
$$
So what is $U$ exactly? Well, it is a matrix parameter. It is the very matrix that will be **learned** to solve our problem. Because $U$ is learnable, I enclosed it in the first block in the figure. In the case of a binary classification problem (e.g. determining whether or not there is a cat in the picture), the output of the neural network needs to be a single value, $U$ has only one column, which essentially makes it a vector. Let's note that in statistics, such a model is a what we call a *logistic regression* ("logit" to those in the know), but without an intercept term.

The next question you might have is why multiply $X$ and $U$? Because it is a way to assign different weights or degrees of importance to each variable/descriptor/feature found in $X$, and finally combine them. For example, a variable related to the presence of fur should have a positive and high contribution for detecting cats in a picture, but a negative and high contribution for detecting dolphins. On the other hand, a variable related to the overall brightness of the picture should have a low or near-zero contribution, as it should (in principle) not reliably distinguish between cats and other animals, for example.

Finally, $\sigma$ is an activation function defined as
$$
\left( \sigma(X) \right)_{ij} = \frac{1}{1 + e^{-X_{ij}}}.
$$
As can be seen immediately from the formula, the function is applied to each element of the matrix *independently*. Here, the point is not to couple the variables, but to *transform* them. To be more specific, this function forces the output to be bounded between 0 and 1. This is very convenient if the model is designed for estimating probabilities (e.g., the probability that a pictured animal is a cat, a dog, a naked mole-rat or a tardigrade). As we will see in the next section, the activation function can have a second, much more important role in the model. This is related to the fact that the function is *non-linear*. Non-linear means that it cannot be represented as a straight line. Here is the sigmoid function:
...
<ResponsiveImage
  ...
  alt="MLP forward pass"
/>

A variant of this two-layer architecture has been proven to be suitable for approximating almost any function, as long as the size of the matrices is large enough. *To be more accurate, it can approximate continuous functions with compact support under specific conditions, but this doesn't matter much.* This important result, called the Universal Approximation Theorem [^3], is a theoretical foundation for using neural networks in diverse and unrelated problems: neural networks are flexible enough to approximate a very wide range of patterns, provided the architecture is large enough and training works well.

Let's note an important transformation of our model. Previously, we had a matrix parameter $U$ of shape $m \times c$, where $c$ is the number of classes (e.g., the number of animal species to distinguish). Now, we have two matrices $U$ and $V$, which have shapes $m \times h$ and $h \times c$, where $h$ is unknown. Intuitively, $h$ controls how many intermediate features the model is allowed to invent before producing the final prediction. In fact, $h$ is unknown because it is up to us to choose it. Because it is arbitrary, it is called a *hyper-parameter*. Let's note that $h$ is the number of columns in $U$ but also the number of rows in $V$ to ensure that the dimensions match. Otherwise, matrix multiplication would be invalid and undefined.

While the theory suggests that this two-layer neural network is extremely powerful, there is a catch: such shallow architectures are rarely sufficient for modern large-scale tasks. You probably came across the term "deep learning" already. "Deep" refers to the number of layers that a neural network possesses. Most modern neural networks have very large numbers of layers. GPT-3 175B, a relatively old large language model, has no fewer than 96 layers [^4]. So why do practitioners have recourse to more layers than just two?
...
A good way to build abstraction is to force the model to reuse parameters and learn intermediate representations instead of memorizing every training example. In other words: **doing more with less**. This is why machine learning research didn't simply stop with the Universal Approximation Theorem. Researchers are constantly seeking to build more powerful models using "sophisticated" mathematical functions. Here are two examples:
...
Convolutions reuse the same filters across an image, so the number of learned parameters does not grow with the number of pixels. Transformers also reuse the same learned projections across sequence positions, although their computation and memory cost still grow with sequence length.
...
It is easy to see that $\ell$ approaches 0 when the model assigns probabilities very close to the correct labels: $Y_{ij} \simeq \hat{Y}_{ij}$ for each $i$ and $j$ (which never happens in practice, for multiple reasons). On the other hand, $\ell$ can increase dramatically fast when $Y_{ij}$ and $\hat{Y}_{ij}$ start to differ.
...
Mathematically, it does not change anything, the new "Sigmoid$_2$ + Loss" block basically performs the two operations in a row, exactly as previously. The change is only conceptual, but will help us in the next section at two levels:
...
Backpropagation builds on the idea of the backward pass, which answers this basic question: "if the error changes a little, how much responsibility does each block has in this change?". Each block receives a gradient from the block after it, uses its own derivatives to transform that gradient, and passes the result to the previous block. This is backpropagation.

### The chain rule and the backward pass

Do you remember how derivatives work? A partial derivative $\frac{\partial y}{\partial x}$ quantifies how much $y$ varies when $x$ varies by a small proportion. Assuming $z$ is a variable that depends on $y$, which in turn depends on $x$, the **chain rule**[^8] provides a formula for how $z$ is affected when $x$ changes:
$$
...
<ResponsiveImage
  ...
  alt="Simplified forward and backward passes"
/>
...
I represented the information passed from block to block as matrices $G^{(1)}$, $G^{(2)}$, $G^{(3)}$ and $G^{(4)}$. Let's apply the **chain rule** to find out what each of these matrices is. That's where the math starts to kick in.

### The "Sigmoid + Loss" building block

Let's start from the end, and compute the gradient of $\ell$ with respect to $Z_{ij}^{(3)}$. This is straightforward because $\ell$ depends on nothing but $Z^{(3)}$. As a reminder, the last block combines both the sigmoid function and the binary cross-entropy function:
$$
...
- The **forward pass**: how to compute the loss function from an input, shown by the arrows going from left to right in the figure below.
- The **backward pass**: how to compute the gradient of that input, shown by the arrow going from right to left.
To make it more obvious that the math is the same regardless of the rest of the architecture of the model, I dropped the $^{(3)}$ from $Z^{(3)}$ in the figure:

<ResponsiveImage
  folder="/images/backpropagation-algorithm"
  filename="backprop-final-block.svg"
  alt="Forward pass and backward pass of the final block"
/>
...
The next step is computing the gradient of $\ell$ with respect to $Z^{(2)}$. Again, it is straightforward because $Z^{(3)}$ depends on nothing but $Z^{(2)}$. Thanks to the chain rule, we know that $G^{(3)}$ relies on $G^{(4)}$. In other words, the derivative $\frac{\partial \ell}{\partial Z_{ij}^{(2)}}$ is necessarily based on the derivative $\frac{\partial \ell}{\partial Z_{ij}^{(3)}}$ which we computed in the previous section:
...
<ResponsiveImage
  ...
  alt="Forward pass and backward pass of the Fully Connected block"
/>
...
<ResponsiveImage
  ...
  alt="Forward pass and backward pass of the Sigmoid block"
/>

The forward and backward passes for the Sigmoid building block are defined in BGD as well, if you are interested in the implementatio details:
...
<ResponsiveImage
  folder="/images/backpropagation-algorithm"
  filename="full.svg"
  alt="MLP forward pass + backward pass"
/>
...
I hope I haven't lost you along the way, because the time has finally come to provide the big picture of how neural networks are actually trained. We already discussed all the steps involved in the training, but the training algorithm can come in different flavors. The most straightforward approach is called "gradient descent", and consists in repetitively updating the parameters by following the direction opposite to the gradient of the loss function:
...
- The other tasks that can be handled by neural networks. I presented how to solve a classification problem, but omitted the other main task: **regression**. Don't worry, the two tasks are very similar and the math presented here can be trivially extended to the regression case.
